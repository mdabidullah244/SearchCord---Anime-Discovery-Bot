...
class SetUpAnimeCog(commands.Cog):
    def __init__(self, client):
        self.client = client

    @app_commands.command(
        name = 'set_up_anime_info',
        description = 'Set up Anime Info To Your Profile'
    )
    @app_commands.checks.cooldown(1, 15.0, key=lambda i:(i.user.id))
    async def set_up_anime_info(self, ctx: commands.Context):
        ...
        try:
            ...
            if data:
                class set_up_anime_info(discord.ui.View):
                    def __init__(self):
                        super().__init__(timeout = None)
                    @discord.ui.button(
                        label = 'Set Up Anime Info',
                        style = discord.ButtonStyle.green
                    )
                    async def set_up_anime_info(self, interaction: discord.Interaction, Button: discord.ui.Button):
                        if interaction.user.id == ctx.user.id:
                            class set_up_anime_modal(discord.ui.Modal, title = 'Set Up Anime Info'):
                                ...
                                async def on_submit(self, interaction: discord.Interaction):
                                    ...
                                    if data:
                                        ...
                                        embed.set_author(icon_url = interaction.user.avatar, name = 'Profile | Anime Info Already Set Up')
                                        embed.set_footer(text = random.choice(random_footer_text))
                                        await interaction.response.edit_message(embed = embed, view = None)
                                    else:
                                        if anime_watched_hours.isdigit():
                                            embed = discord.Embed(
                                                color = discord.Color.from_str(random.choice(randomstrcolor)),
                                                timestamp = interaction.created_at
                                            )
                                            embed.set_author(icon_url = interaction.user.avatar, name = 'Profile | Anime | Set Up Complete')
                                            embed.set_footer(text = random.choice(random_footer_text))
                                            await interaction.response.edit_message(embed = embed, view = None)
                                            cursor.execute('INSERT INTO profile_db_anime_info (username, fav_anime, anime_watched_name, anime_watching_name, anime_to_watch_name, anime_watched_hours) VALUES (?, ?, ?, ?, ?, ?)', (username, fav_anime, anime_watched_name, anime_watching_name, anime_to_watch_name, anime_watched_hours))
                                            connection.commit()
                                        else:
                                            embed = discord.Embed(
                                                color = discord.Color.from_str(random.choice(randomstrcolor)),
                                            ...
                                            embed.set_footer(text = random.choice(random_footer_text))
                                            await interaction.response.edit_message(embed = embed, view = None)
                            await interaction.response.send_modal(set_up_anime_modal())
                        else:
                            embed = discord.Embed(
                ...
                embed.set_author(icon_url = ctx.user.avatar, name = f'Profile | Set Up Anime Info')
                embed.set_footer(text = random.choice(random_footer_text))
                await ctx.response.send_message(embed = embed, view = set_up_anime_info())
            else:
                embed = discord.Embed(
                    color = discord.Color.from_str('#eee0ca'),
                    timestamp = ctx.created_at
            ...
            connection.close()
    @set_up_anime_info.error
    async def set_up_anime_info_error(self, ctx: commands.Context, error: Exception):
...
async def setup(client):
    await client.add_cog(SetUpAnimeCog(client))